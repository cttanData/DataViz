...
Generally, the data set seemed to be very time series focus and attempting to determine relationship. Thus from a visualisation perspective, it would be good if [timeviz](https://github.com/daattali/timevis) and [visNetwork](https://datastorm-open.github.io/visNetwork/) based on network, graph and time dimension could be leveraged. However, such would required certain level of technical skillset. 
...
visNetwork(pers_nodes,gccard_edges) %>%
  ...
  visGroups(groupname = "Info Tech", color="light green", 
            shape = "circle",
            shadow = list(enabled = TRUE)) %>%
...
DT::datatable( GAStech_node %>% 
  dplyr::select(-PersID, -FirstName,-LastName, -Loyalty, -Appt),
  filter ='top',class = 'cell-border stripe',rownames=FALSE,
  options = list(
            columnDefs = list(list(className = 'dt-center',
                                   targets = "_all"))
...
visNetwork(pr_nodes,pr) %>%
  ...
  visGroups(groupname = "Security", color="red",
            shape = "square", 
            shadow = list(enabled = TRUE)) %>% 
  visGroups(groupname = "Info Tech", color="green", 
            shape = "circle",
            shadow = list(enabled = TRUE)) %>%
...
g <- fjoin %>%
  ...
  scale_x_continuous(breaks=seq(6,19,1)) + 
  labs(title = expression(underline(
    "Date of Transaction & Count at Brewed
    Awakenings with Facet(Credit Card No)")),
       y="No. of Transaction",
       x=" X Jan 2014",
       fill="Department") + 
  facet_grid(row = vars(ccard), scales="free_y", space="free_y")+  
...
g <- mgps %>%
  ...
  geom_tile_interactive(aes(tooltip = time, data_id = location),
                        colour="white", width=0.5)+
  facet_grid(id~.,scale="free")+
  labs(title = "Visit to Industry Locations by Veh ID (Facet).",
       y="",
       x="Day",
       fill="Location") + 
  theme_minimal()+ 
...
g <- mgps %>%
  ...
  geom_tile_interactive(aes(tooltip = time, data_id = location),
                        colour="white", width=0.5)+
  facet_grid(id~.,scale="free")+
  labs(title = "Locations Visted by Veh ID (Facet).",
       y="",
       x="Day",
       fill="Location") + 
  theme_minimal()+ 
