...
```{r, layout="l-body",fig.height = 6, fig.align='centre', warning = FALSE }
...
Reference to location setting stipulated in section 5.3, a bar chart stipulated on location type was shown below. Code chuck was relatively similar to the above less some changes in the parameters. 
From 
```{r, echo=FALSE }
data <- mgps %>% count(id, loctype) 
fig <- plot_ly(data, x = ~reorder(id,-n,sum), y = ~n, type = 'bar', name = ~loctype,
               colours=c("grey","sky blue","blue", "pink", "orange", "green"))
