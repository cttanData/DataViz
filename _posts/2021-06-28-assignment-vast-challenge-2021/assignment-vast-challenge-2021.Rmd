...
knitr::opts_chunk$set(message = FALSE, fig.retina = 3,layout="l-body", fig.height = 6*0.618, fig.align='centre')
```

```{r echo = FALSE, warning = FALSE}
packages <- c('sf','tidyverse', 'igraph', 'plotly', 'ggiraph','lubridate', 'clock', 'ggraph',"leaflet", 'tmap', 'visNetwork', 'tidygraph', "timevis", "DT")
for (p in packages){
...
gps_sf <- st_as_sf(cgps, coords = c("long","lat"), crs = 4326)
...
```{r message = FALSE,layout="l-body", warning = FALSE}
...
```{r}
timevis(data = visfull_ds, groups=vis_gp, height="400px", width = "700px")
```
...
```{r, echo=TRUE}
datatable( mgps %>% 
             dplyr::select(-count, -loctype, -lat, -long) %>% arrange(id,trip),
           rownames = FALSE,
           filter ='top',
           options = list(
             columnDefs = list(list(className = 'dt-center', targets = "_all"))
            ),
           colnames = c("Vehicle ID","Trip ID","Timestamp", "Location")) %>%
  formatStyle(0, target ='row',color = 'Black', backgroundColor = 'light green', lineHeight ='70%')
```
...
```{r}
datatable( fjoin %>% 
  dplyr::select(location, price,last4ccnum, loyaltynum, cardtype),
  #filter(cardtype !="both") %>%
  filter ='top',class = 'cell-border stripe',rownames=FALSE,
  options = list(
            columnDefs = list(list(className = 'dt-center', targets = "_all"))
            ),
  colnames = c("Location","Price($)","Credit Card No.", "Loyalty Card No.", "Transacted Card")) %>%
  formatStyle(0, target ='row',color = 'Black', backgroundColor = 'light green', lineHeight ='70%')
```
...
```{r fig.height=6*0.618, fig.width=6, paged.print=TRUE}
# filter and grouping code chuck for timevis
...
datatable( mgps %>% filter(id==20) %>%
  ...
  formatStyle(0, target ='row',color = 'Black', backgroundColor = 'light green', lineHeight ='70%')
```
2. __Individual Card Transactions__. For individual cards transacted on the same day, most of them were deemed to be valid single transaction when verified with geospatial tracking data of the card holders. The difference in price should be verified with the stores as there could some promotion charge rate for loyalty card. eg one could check the transaction at Brewed Served in the above mentioned on 9 Jan when the vehicle did as usual travel to the locality in the morning. Nonetheless, there were also transactions in particular on loyalty card only transaction that could not be verified with geospatial tracking data.

3. __Present or Not Present__. With geospatial data, new insights were discerned. i.e. geospatial records at locations but no transaction records. Technically, many possible scenarios. There could be carpooled where the card holders present at the location with expenses or vice verse someone paid for the card holder meal where he droved to the location. one could check on 15 Jan on the above mentioned where the owner drove to Guy's Gyros, stay there for 40mins without transactions. 

```{r echo =FALSE}
g <- fjoin %>% filter(ccard  %in% c("1286")) %>% arrange(ccard, timestamp) %>%
  separate(timestamp, into = c("dat","time"), sep=" ",remove = FALSE) %>%
  ggplot(aes(x = reorder(location, hr), fill=cardtype)) + 
  geom_bar_interactive(aes(tooltip = time), width=0.5) + 
  facet_grid(ccard~day, scale = 'free') +
...
```{r echo =FALSE}
g <- mgps %>% filter(id  %in% c("22")) %>%
  group_by(id,trip) %>% 
  summarise (count =n(),timestamp = first(timestamp), location = first(location), loctype=first(loctype)) %>%
  filter( !(location %in% c("GASTech", "ID 22/23/30"))) %>%
  separate(timestamp, into = c("date","time"), sep=" ",remove = TRUE) %>%
  ...
  #scale_fill_manual(values=c("#1569C7", "orange", "green"))+
  scale_y_continuous(breaks=seq(0,2,1)) +
  labs(title = "Traveled Locations for Vehicle ID.",
       y="No. of Transaction",
       x="",
       fill="Card Type") + 
  theme_classic()+ 
