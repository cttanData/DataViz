...
## 3.1 Assumption
The intention is to provide insights on the marco view for total trade transaction values for Singapore and it top 6 trading countries in 2020 as compared to 2019. The visualisation will focus on Year-over-Year growth by comparing on total trade values, respective import and export values in 2020 baseline to 2019. Trend lines will might also be derived but it will not focus on driving insights on trade balance with individual countries.
...
![*Figure4.1. Proposed Visualisation Design.](img/DVMO2.png){ width=100% }
</center>
...
26|Create the calulated field for "Annual Growth" a logic measure to determine whether Trade Value in 2020 is more than Trade Value in 2019. Then calculated field for "Annual Growth%" reference to the image. The purpose is to compute annual Year-over-Year growth for each country.|![](img/SbS25.png){width=160%}
...
29|Create a Year-over-Year Growth for total trade values on each month in 2020. On the new sheet, drag "Country" and "Date" into Columns Shelf with the required filter and setting to month. drag "Annual Growth%" into Marks Shelf Colour and Size respectively as Measure. This is to create the visual cue for negative / positive values with size to indicate the difference magnitude.|![](img/SbS28.png){width=160%}
...
1. **Mostly Negative in Growth**. The Year-over-Year growth for the total trade values for SG is negative (i.e-5.20%) followed by it main trading partners in descending order Malaysia (-8.46%), USA (-2.47%), Japan(-1.69%) and China (-0.81%). Nonetheless it is surprising that SG has strong positive growth with Taiwan (+13.04%) and Hong Kong (+5.61%). Moreover, the negative growth in China is deemed to be low given it the country is hit with pandemic in early 2020.

2.  **Uneven Impact to Monthly Import and Export Values for the Top 6 Countries**. 

+ **China**. Trade is affected generally more on export than import during the first 6 mths of 2020. Thereafter, negative import growth but overall trade growth is postivie due to higher growth in export.

+ **Malaysia**. Badly hit during Mar to May 2020 which also the period where both implemented lock down such that direct land trade via Johor has been affected. As Malaysia has been implementing a series lock down even after that, one can observe the continue impact to Export from SG to Malaysia. Overall trade growth has been negative in the last 6 mths of 2020.

+ **USA**. 

# 7.0 Conclusion

The visualisation has provided a macro view of total merchandised trade values for SG and its top 6 trading partner. A quick insights on the Year-over-Year growth on annual trade values, import and export values for each trading country with SG under the pandemic COVID 19. It also shed marco insight on monthly growth in 2020 showing signs of recovery for selected countries. Nonetheless, the future is still uncertain in this pandemic era as country about subsequent COVID attack. Effectively, trade economics can be affected by numerous factors and thus without deep research,one cannot confidently concluded the negative growth can be fully attributed to the pandemic while selected incidents with Malaysia are attributable.

# 8.0 Recommendation

Further studies shall gain better understanding on the negative growth can focus on 2 areas as follows:

1. Micro View on Merchandised Trade. Focus research on 

2. Causation to Growth in Merchandised Trade. COVID 19 can be a causation to the growth insights determined from the visualisation at the macro leve. However, other political or econmicial factors could also have impacted how good and services flow globally. eg Trade wars between US and China, Hong Kong political instability, Taiwan growth in electronic supply chain etc  could have affected golbal trade flow. Need to have deeper research into this area to understand the impact to Singapore Trade. 
